...
/**
 * Defines the calculation engine for duration fields. The interface defines a
 * set of methods that manipulate a millisecond duration with regards to a
 * single field, such as months or seconds.
 * <p>
 * This design is extensible so, if you wish, you can extract a different field
 * from the millisecond duration. A number of standard implementations are
 * provided to assist.
 *
...
public abstract class DurationField implements Comparable<DurationField> {
	 ...
	 * milliseconds without needing a reference date. Put another way, a precise
	 * field's unit size is not variable.
	 * 
	 ...
	 *             if the value is too large for an int
	 */
	 ...
	 * For precise fields this method produces the same result as for the single
	 * argument get method.
	 * <p>
	...
	// * unit millisecond order. This ordering is inconsistent with equals, as
	// it
	// * ignores name and precision.
	// *
	// * @param durationField a duration field to check against
	// * @return negative value if this is less, 0 if equal, or positive value
	// if greater
	 ...
	 * @param locale
	 *            the locale to use for selecting a name, null for default
	 */
	 ...
	 * 
	 * @param locale
	 *            the locale to use for selecting a unit name, null for default
