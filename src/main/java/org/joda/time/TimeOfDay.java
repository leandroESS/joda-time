...
/**
 * TimeOfDay is an immutable partial supporting the hour, minute, second and
 * millisecond fields.
 * <p>
 * NOTE: This class only supports the four fields listed above. Thus, you cannot
 * query the millisOfDay or secondOfDay fields for example. The new
 * <code>LocalTime</code> class removes this restriction.
 * <p>
 * Calculations on TimeOfDay are performed using a {@link Chronology}. This
 ...
 * </ul>
 * <p>
 * TimeOfDay is thread-safe and immutable, provided that the Chronology is as
...
public final class TimeOfDay extends BasePartial implements ReadablePartial, Serializable {
	...
	private static final DateTimeFieldType[] FIELD_TYPES = new DateTimeFieldType[] { DateTimeFieldType.hourOfDay(),
			DateTimeFieldType.minuteOfHour(), DateTimeFieldType.secondOfMinute(), DateTimeFieldType.millisOfSecond(), };
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs a TimeOfDay from a <code>java.util.Calendar</code> using
	 ...
	 * created TimeOfDay no matter what the time zone is. For example, if the
	 * Calendar states that the time is 04:29, then the created TimeOfDay will
	 ...
	 * @throws IllegalArgumentException
	 *             if the time is invalid for the ISO chronology
	 * @since 1.2
	 */
	...
	/**
	 * Constructs a TimeOfDay from a <code>java.util.Date</code> using exactly
	 * the same field values avoiding any time zone effects.
	 * <p>
	 * Each field is queried from the Date and assigned to the TimeOfDay. This
	 * is useful to ensure that the field values are the same in the created
	 * TimeOfDay no matter what the time zone is. For example, if the Calendar
	 * states that the time is 04:29, then the created TimeOfDay will always
	 * have the time 04:29 irrespective of time zone issues.
	 * <p>
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs a TimeOfDay from the specified millis of day using the ISO
	 * chronology.
	 * <p>
	 * The millisOfDay value may exceed the number of millis in one day, but
	 * additional days will be ignored. This method uses the UTC time zone
	 * internally.
	 *
	 * @param millisOfDay
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs a TimeOfDay with the current time, using ISOChronology in the
	 * default zone to extract the fields.
	 * <p>
	 * The constructor uses the default time zone, resulting in the local time
	 * being initialised. Once the constructor is complete, all further
	 * calculations are performed without reference to a timezone (by switching
	 * to UTC).
	 */
	public TimeOfDay() {
		super();
	...
	/**
	 * Constructs a TimeOfDay with the current time, using ISOChronology in the
	 * specified zone to extract the fields.
	 * <p>
	...
	/**
	 * Constructs a TimeOfDay with the current time, using the specified
	 * chronology and zone to extract the fields.
	 * <p>
	 * The constructor uses the time zone of the chronology specified. Once the
	 * constructor is complete, all further calculations are performed without
	 * reference to a timezone (by switching to UTC).
	 *
	 * @param chronology
	 ...
	 * The constructor uses the time zone of the chronology specified. Once the
	 * constructor is complete, all further calculations are performed without
	 * reference to a timezone (by switching to UTC). The specified chronology
	...
	/**
	 * Constructs a TimeOfDay with specified hour and minute and zero seconds
	 * and milliseconds using <code>ISOChronology</code> in the default zone.
	 * <p>
	 * The constructor uses the no time zone initialising the fields as
	 * provided. Once the constructor is complete, all further calculations are
	 * performed without reference to a timezone (by switching to UTC).
	 *
	 * @param hourOfDay
	 *            the hour of the day
	 * @param minuteOfHour
	...
	// @ requires hourOfDay <= 24;
	// @ requires minuteOfHour >= 0;
	// @ requires minuteOfHour <= 59;
	...
	/**
	 * Constructs a TimeOfDay with specified time field values and zero
	 * milliseconds using <code>ISOChronology</code> in the default zone.
	 * <p>
	 * The constructor uses the no time zone initialising the fields as
	 * provided. Once the constructor is complete, all further calculations are
	 * performed without reference to a timezone (by switching to UTC).
	...
	/**
	 * Constructs a TimeOfDay with values from this instance and a new
	 * chronology.
	 *
	 * @param partial
	 *            the partial to base this new instance on
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a copy of this time with the specified chronology. This instance
	...
	/**
	 * Returns a copy of this time with the value of the specified field
	 * increased, wrapping to what would be a new day if required.
	 * <p>
	...
	/**
	 * Returns a copy of this time with the specified period added, wrapping to
	 * what would be a new day if required.
	 * <p>
	 * If the addition is zero, then <code>this</code> is returned. Fields in
	 ...
	 * field is best achieved using methods like {@link #plusHours(int)}.
	 * 
	 * @param period
	 ...
	 * @throws ArithmeticException
	 *             if the new time exceeds capacity
	 */
	...
	public LocalTime toLocalTime() {
		return new LocalTime(getHourOfDay(), getMinuteOfHour(), getSecondOfMinute(), getMillisOfSecond(),
				getChronology());
	}
	 ...
	 * setting the time fields from this instance and the date fields from the
	 * current time.
	 *
	 ...
	 * <p>
	 * TimeOfDay is immutable, so there are no set methods. Instead, this method
	 ...
	 * <p>
	 * This class binds a <code>TimeOfDay</code> to a
	...
	public static class Property extends AbstractPartialFieldProperty implements Serializable {
		 ...
		 * to what would be the next day if necessary.
		 * <p>
		...
		/**
		 * Adds to the value of this field in a copy of this TimeOfDay, throwing
		 * an Exception if the bounds are exceeded.
		 * <p>
		 * The value will be added to this field. If the value is too large to
		 * be added solely to this field then it will affect larger fields.
		 * Smaller fields are unaffected.
		 * <p>
		 * If the result would be too large (beyond 23:59:59:999) or too small
		 * (less than 00:00:00.000) then an Exception is thrown. For the
		 * alternate behaviour which wraps to the next 'day', see
		 * {@link #addToCopy(int)}.
		 * <p>
		 * The TimeOfDay attached to this property is unchanged by this call.
		 * Instead, a new instance is returned.
		 ...
		 * <p>
		 * For example, <code>12:59:37</code> addWrapField one minute returns
		 * <code>12:00:37</code>.
		 * <p>
		 * The TimeOfDay attached to this property is unchanged by this call.
		 * Instead, a new instance is returned.
		...
		// -----------------------------------------------------------------------
		/**
		 * Returns a new TimeOfDay with this field set to the maximum value for
		 * this field.
		 * <p>
		 * The TimeOfDay attached to this property is unchanged by this call.
		 *
		...
		/**
		 * Returns a new TimeOfDay with this field set to the minimum value for
		 * this field.
		 * <p>
		 * The TimeOfDay attached to this property is unchanged by this call.
		 *
