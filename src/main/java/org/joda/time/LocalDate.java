 ...
 * zone.
 * <p>
 * LocalDate implements the {@link ReadablePartial} interface. To do this, the
 * interface methods focus on the key fields - Year, MonthOfYear and DayOfMonth.
 * However, <b>all</b> date fields may in fact be queried.
 * <p>
 * LocalDate differs from DateMidnight in that this class does not have a time
 * zone and does not represent a single instant in time.
 * <p>
 * Calculations on LocalDate are performed using a {@link Chronology}. This
 * chronology will be set internally to be in the UTC time zone for all
 * calculations.
 *
 * <p>
 ...
 *
 * <p>
 * LocalDate is thread-safe and immutable, provided that the Chronology is as
...
public final class LocalDate extends BaseLocal implements ReadablePartial, Serializable {
	...
	public static LocalDate parse(/* @ non_null @ */String str) {
	...
	public static LocalDate parse(/* @ non_null @ */String str, DateTimeFormatter formatter) {
		return formatter.parseLocalDate(str);
	}

	// -----------------------------------------------------------------------
	/**
	 * Constructs a LocalDate from a <code>java.util.Calendar</code> using
	 ...
	 * version of the time zone data differs between the JDK and Joda-Time. That
	 * is because the local field values are transferred, calculated using the
	 * JDK time zone data and without using the Joda-Time time zone data.
	 * <p>
	 * This factory method ignores the type of the calendar and always creates a
	 * LocalDate with ISO chronology. It is expected that you will only pass in
	 * instances of <code>GregorianCalendar</code> however this is not
	 * validated.
	 *
	 * @param calendar
	 ...
	 */
	public static LocalDate fromCalendarFields(/* @ non_null @ */Calendar calendar) {
		if (calendar == null) {
	...
	/**
	 * Constructs a LocalDate from a <code>java.util.Date</code> using exactly
	 * the same field values.
	 * <p>
	 * Each field is queried from the Date and assigned to the LocalDate. This
	 * is useful if you have been using the Date as a local date, ignoring the
	 * zone.
	 * <p>
	 * One advantage of this method is that this method is unaffected if the
	 * version of the time zone data differs between the JDK and Joda-Time. That
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs an instance set to the current local time evaluated using ISO
	 * chronology in the default zone.
	 * <p>
	...
	/**
	 * Constructs an instance set to the current local time evaluated using ISO
	 * chronology in the specified zone.
	 * <p>
	 * If the specified time zone is null, the default zone is used. Once the
	 ...
	 * <p>
	 * If the chronology is null, ISO chronology in the default time zone is
	 * used. Once the constructor is completed, the zone is no longer used.
	 *
	 * @param chronology
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs an instance from an Object that represents a datetime. The
	 * time zone will be retrieved from the object if possible, otherwise the
	 ...
	 * include ReadablePartial, ReadableInstant, String, Calendar and Date. The
	 * String formats are described by
	 * {@link ISODateTimeFormat#localDateParser()}. The default String converter
	 ...
	 * <p>
	 * If the chronology is null, ISO in the default time zone is used. Once the
	 * constructor is completed, the zone is no longer used. If the instant
	 * contains a chronology, it will be ignored. For example, passing a
	 * {@code LocalDate} and a different chronology will return a date with the
	 * year/month/day from the date applied unaltered to the specified
	 * chronology.
	 * <p>
	 * The recognised object types are defined in
	 * {@link org.joda.time.convert.ConverterManager ConverterManager} and
	...
	// @ requires monthOfYear < 13;
	// @ requires  dayOfMonth < 32;
	...
	/**
	 * Constructs an instance set to the specified date and time using the
	 * specified chronology, whose zone is ignored.
	 * <p>
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the number of fields in this partial, which is three. The supported
	 * fields are Year, MonthOfYear and DayOfMonth. Note that all fields from
	 * day and above may in fact be queried via other methods.
	 *
	 ...
	 */
	// @ requires index = YEAR;
	// @ requires index = MONTH_OF_YEAR;
	// @ requires index = DAY_OF_MONTH;
	 ...
	 * Note that all fields from day and above may in fact be queried via other
	 * methods.
	 *
	 * @param index
	 ...
	 *
	 * @param fieldType
	 *            a field type, usually obtained from DateTimeFieldType, not
	 *            null
	...
	/**
	 * Checks if the field type specified is supported by this local date and
	 * chronology. This can be used to avoid exceptions in
	 * {@link #get(DateTimeFieldType)}.
	 *
	 * @param type
	...
	/**
	 * Checks if the duration type specified is supported by this local date and
	 * chronology.
	 *
	 * @param type
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the local milliseconds from the Java epoch of 1970-01-01T00:00:00
	 * (not fixed to any specific time zone).
	 * 
	...
	// -----------------------------------------------------------------------
	/**
	 * Compares this ReadablePartial with another returning true if the
	...
	/**
	 * Compares this partial with another returning an integer indicating the
	 * order.
	 * <p>
	 * The fields are compared in order, from largest to smallest. The first
	 * field that is non-equal is used to determine the result.
	 * <p>
	 * The specified object must be a partial instance whose field types match
	 * those of this partial.
	 *
	 * @param partial
	 *            an object to check against
	 * @return negative if this is less, zero if equal, positive if greater
	 * @throws ClassCastException
	 *             if the partial is the wrong class or if it has field types
	 *             that don't match
	...
	// -----------------------------------------------------------------------
	/**
	 * Converts this LocalDate to a full datetime at the earliest valid time for
	 * the date using the default time zone.
	 * <p>
	...
	/**
	 * Converts this LocalDate to a full datetime at the earliest valid time for
	 * the date using the specified time zone.
	 * <p>
	 * The time will normally be midnight, as that is the earliest time on any
	 * given day. However, in some time zones when Daylight Savings Time starts,
	 * there is no midnight because time jumps from 11:59 to 01:00. This method
	 ...
	 * time zone.
	 * <p>
	 * This method will throw an exception if the default time zone switches to
	 * Daylight Savings Time at midnight and this LocalDate represents that
	 * switchover date. The problem is that there is no such time as midnight on
	 * the required date, and as such an exception is thrown.
	 * <p>
	 * This instance is immutable and unaffected by this method call.
	 * 
	 * @return this date as a datetime at midnight
	 * @deprecated Use {@link #toDateTimeAtStartOfDay()} which won't throw an
	 *             exception
	 */
	@Deprecated
	 ...
	 * specified time zone.
	 * <p>
	 * This method will throw an exception if the time zone switches to Daylight
	 * Savings Time at midnight and this LocalDate represents that switchover
	 * date. The problem is that there is no such time as midnight on the
	 * required date, and as such an exception is thrown.
	 * <p>
	 * This method uses the chronology from this instance plus the time zone
	 * specified.
	 * <p>
	 ...
	 *            the zone to use, null means default zone
	 * @return this date as a datetime at midnight
	 * @deprecated Use {@link #toDateTimeAtStartOfDay(DateTimeZone)} which won't
	 *             throw an exception
	 */
	@Deprecated
	 ...
	 * current time.
	 * <p>
	 * This method will throw an exception if the datetime that would be created
	 * does not exist when the time zone is taken into account.
	 * <p>
	 * This instance is immutable and unaffected by this method call.
	 *
	...
	// -----------------------------------------------------------------------
	/**
	 * Converts this object to a LocalDateTime using a LocalTime to fill in the
	 * missing fields.
	 * <p>
	 * The resulting chronology is determined by the chronology of this
	 * LocalDate. The chronology of the time must also match. If the time is
	 ...
	 * {@link #toDateTimeAtCurrentTime(DateTimeZone)} and the following
	 * documentation does not apply.
	 * <p>
	 * When the time zone is applied, the local date-time may be affected by
	 * daylight saving. In a daylight saving gap, when the local time does not
	 * exist, this method will throw an exception. In a daylight saving overlap,
	 * when the same local time occurs twice, this method returns the first
	 * occurrence of the local time.
	 * <p>
	 * This instance is immutable and unaffected by this method call.
	 *
	 * @param time
	 ...
	 *             if the local time does not exist when the time zone is
	 *             applied
	 */
	...
	public DateTime toDateTime(LocalTime time, DateTimeZone zone) {
		...
		Chronology chrono = getChronology().withZone(zone);
		return new DateTime(getYear(), getMonthOfYear(), getDayOfMonth(), time.getHourOfDay(), time.getMinuteOfHour(),
	 ...
	 * <p>
	 * The <code>Date</code> object created has exactly the same year, month and
	 * day as this date. The time will be set to the earliest valid time for
	 * that date.
	 * <p>
	 ...
	 * This method works by taking a first guess and then adjusting the JDK date
	 * until it has the earliest valid instant. This also handles the situation
	 ...
	 * <p>
	 * The returned object will be a new instance of the same type. Only the
	 * millis will change, the chronology is kept. The returned object will be
	 * either be a new instance or <code>this</code>.
	 *
	 * @param newMillis
	 *            the new millis, from 1970-01-01T00:00:00
	 ...
	 * fields will be changed in the returned instance. Unsupported fields are
	 * ignored. If the partial is null, then <code>this</code> is returned.
	 *
	 * @param partial
	 ...
	 * of year field will be changed in the returned instance. If the field type
	 * is null, then <code>this</code> is returned.
	 * <p>
	...
	/**
	 * Returns a copy of this date with the value of the specified field
	 * increased.
	 * <p>
	 ...
	 * <p>
	 * This adds the specified number of years to the date. If adding years
	 * makes the day-of-month invalid, it is adjusted to the last valid day in
	 * the month. This LocalDate instance is immutable and unaffected by this
	 * method call.
	 * <p>
	 ...
	 * <p>
	 * This adds the specified number of months to the date. The addition may
	 * change the year, but the day-of-month is normally unchanged. If adding
	 * months makes the day-of-month invalid, it is adjusted to the last valid
	 * day in the month. This LocalDate instance is immutable and unaffected by
	 * this method call.
	 * <p>
	 * The following three lines are identical in effect:
	 * 
	 ...
	 * {@link #minusYears(int)}.
	 * <p>
	 * Unsupported time fields are ignored, thus subtracting a period of 24
	 * hours will not have any effect.
	 *
	 * @param period
	 ...
	 * <p>
	 * This subtracts the specified number of years from the date. If
	 * subtracting years makes the day-of-month invalid, it is adjusted to the
	 * last valid day in the month. This LocalDate instance is immutable and
	 ...
	 * <p>
	 * This subtracts the specified number of months from the date. The
	 * subtraction may change the year, but the day-of-month is normally
	 * unchanged. If subtracting months makes the day-of-month invalid, it is
	 * adjusted to the last valid day in the month. This LocalDate instance is
	 ...
	 * In the standard ISO8601 week algorithm, the first week of the year is
	 * that in which at least 4 days are in the year. As a result of this
	 ...
	 * <p>
	 * LocalDate is immutable, so there are no set methods. Instead, this method
	...
	/**
	 * Get the weekyear property which provides access to advanced
	 * functionality.
	 *
	 ...
	 * instances of LocalDate - they do not modify the original. The example
	...
	public static final class Property extends AbstractReadableInstantFieldProperty {
		...
		/**
		 * Adds to this field, possibly wrapped, in a copy of this LocalDate. A
		 * field wrapped operation only changes this field. Thus 31st January
		...
		// -----------------------------------------------------------------------
		/**
		 * Returns a new LocalDate with this field set to the maximum value for
		 * this field.
		 * <p>
		 * This operation is useful for obtaining a LocalDate on the last day of
		...
		/**
		 * Returns a new LocalDate with this field set to the minimum value for
		 * this field.
		 * <p>
		 * The LocalDate attached to this property is unchanged by this call.
		 *
		 ...
		 * where the time is 10:30 would result in new LocalDate with the time
		 * of 10:00.
		 *
		 * @return a copy of the LocalDate with the field value changed
		 */
