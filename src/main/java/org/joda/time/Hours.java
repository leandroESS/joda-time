 ...
 * <p>
 * <code>Hours</code> is an immutable period that can only store hours. It does
 * not store years, months or minutes for example. As such it is a type-safe way
 * of representing a number of hours in an application.
 * <p>
...
public final class Hours extends BaseSingleFieldPeriod {
	...
	/**
	 * Constant representing the maximum number of hours that can be stored in
	 * this object.
	 */
	public static final Hours MAX_VALUE = new Hours(Integer.MAX_VALUE);
	/**
	 * Constant representing the minimum number of hours that can be stored in
	 * this object.
	 */
	 ...
	 * specify two <code>LocalTime</code> objects.
	 *
	 * @param start
	...
	public static Hours hoursBetween(ReadablePartial start, ReadablePartial end) {
		if (start instanceof LocalTime && end instanceof LocalTime) {
			...
			int hours = chrono.hours().getDifference(((LocalTime) end).getLocalMillis(),
					((LocalTime) start).getLocalMillis());
	...
	/**
	 * Creates a <code>Hours</code> representing the number of whole hours in
	 ...
	 * standardised durations for each field. Only those fields which have a
	...
	/**
	 * Creates a new <code>Hours</code> by parsing a string in the ISO8601
	 * format 'PTnH'.
	 * <p>
	 * The parse will accept the full ISO syntax of PnYnMnWnDTnHnMnS however
	 * only the hours component may be non-zero. If any other component is
	...
	// -----------------------------------------------------------------------
	/**
	 * Creates a new instance representing a number of hours. You should
	 * consider using the factory method {@link #hours(int)} instead of the
	 * constructor.
	 *
	 * @param hours
	 *            the number of hours to represent
	 */
	// @ requires hours >= 0;
	// @ requires hours <= 8;
	...
	// -----------------------------------------------------------------------
	/**
	 * Converts this period in hours to a period in weeks assuming a 7 day week
	 * and 24 hour day.
	 * <p>
	 * This method allows you to convert between different types of period.
	 * However to achieve this it makes the assumption that all weeks are 7 days
	 * long and all days are 24 hours long. This is not true when daylight
	 * savings time is considered, and may also not be true for some unusual
	 * chronologies. However, it is included as it is a useful operation for
	 * many applications and business rules.
	 * 
	 * @return a period representing the number of whole weeks for this number
	 *         of hours
	 */
	 ...
	 * hours long. This is not true when daylight savings time is considered,
	 * and may also not be true for some unusual chronologies. However, it is
	...
	/**
	 * Converts this period in hours to a period in minutes assuming a 60 minute
	 * hour.
	 * <p>
	 * This method allows you to convert between different types of period.
	 * However to achieve this it makes the assumption that all hours are 60
	 * minutes long. This may not be true for some unusual chronologies.
	...
	/**
	 * Converts this period in hours to a period in seconds assuming a 60 minute
	 * hour and 60 second minute.
	 * <p>
	 * This method allows you to convert between different types of period.
	 * However to achieve this it makes the assumption that all hours are 60
	 * minutes long and all minutes are 60 seconds long. This may not be true
	 * for some unusual chronologies. However, it is included as it is a useful
	...
	// -----------------------------------------------------------------------
	/**
	 * Converts this period in hours to a duration in milliseconds assuming a 60
	 * minute hour and 60 second minute.
	 * <p>
	 * This method allows you to convert from a period to a duration. However to
	 * achieve this it makes the assumption that all hours are 60 minutes and
	 * all minutes are 60 seconds. This might not be true for an unusual
	 * chronology, for example one that takes leap seconds into account.
	 ...
	 *
	 * @param hours
	 *            the amount of hours to take away, may be negative, null means
	 *            zero
	 * @return the new period minus the specified number of hours
	 * @throws ArithmeticException
	 *             if the result overflows an int
