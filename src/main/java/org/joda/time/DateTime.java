 ...
 * {@link ReadableInstant}. As with all instants, it represents an exact point
 * on the time-line, but limited to the precision of milliseconds. A
 * <code>DateTime</code> calculates its fields with respect to a
 ...
 * Secondly, it holds a {@link Chronology} which determines how the millisecond
 * instant value is converted into the date time fields. The default Chronology
 * is {@link ISOChronology} which is the agreed international standard and
 * compatible with the modern Gregorian calendar.
 * <p>
 ...
 * well. All standard Chronology classes supplied are thread-safe and immutable.
 *
...
public final class DateTime extends BaseDateTime implements ReadableDateTime, Serializable {
	 ...
	 * <p>
	 * Sometimes this method and {@code new DateTime(str)} return different
	 * results. This can be confusing as the difference is not visible in
	 * {@link #toString()}.
	 * <p>
	 * When passed a date-time string without an offset, such as
	 * '2010-06-30T01:20', both the constructor and this method use the default
	 * time-zone. As such, {@code DateTime.parse("2010-06-30T01:20")} and
	 * {@code new DateTime("2010-06-30T01:20"))} are equal.
	 * <p>
	 * However, when this method is passed a date-time string with an offset,
	 * the offset is directly parsed and stored. As such,
	 * {@code DateTime.parse("2010-06-30T01:20+02:00")} and
	 * {@code new DateTime("2010-06-30T01:20+02:00"))} are NOT equal. The object
	 * produced via this method has a zone of
	 * {@code DateTimeZone.forOffsetHours(2)}. The object produced via the
	 ...
	 */
	@FromString
	public static DateTime parse(/* @ non null @ */ String str) {
	 ...
	 * @since 2.0
	 */
	public static DateTime parse(/* @ non null @ */ String str, DateTimeFormatter formatter) {
	...
	/**
	 * Constructs an instance set to the current system millisecond time using
	 * <code>ISOChronology</code> in the specified time zone.
	 * <p>
	 ...
	 * <p>
	 * If the chronology is null, <code>ISOChronology</code> in the default time
	 * zone is used.
	 *
	 * @param chronology
	 ...
	 * <p>
	 * If the object implies a chronology (such as GregorianCalendar does), then
	 * that chronology will be used. Otherwise, ISO default is used. Thus if a
	 * GregorianCalendar is passed in, the chronology used will be GJ, but if a
	 * Date is passed in the chronology will be ISO.
	 * <p>
	...
	/**
	 * Constructs an instance from an Object that represents a datetime, forcing
	 * the time zone to that specified.
	 * <p>
	 * If the object implies a chronology (such as GregorianCalendar does), then
	 * that chronology will be used, but with the time zone adjusted. Otherwise,
	 * ISO is used in the specified time zone. If the specified time zone is
	 * null, the default zone is used. Thus if a GregorianCalendar is passed in,
	 * the chronology used will be GJ, but if a Date is passed in the chronology
	 * will be ISO.
	 * <p>
	 * The recognised object types are defined in
	 * {@link org.joda.time.convert.ConverterManager ConverterManager} and
	...
	/**
	 * Constructs an instance from an Object that represents a datetime, using
	 * the specified chronology.
	 * <p>
	 * If the chronology is null, ISO in the default time zone is used. Any
	 * chronology implied by the object (such as GregorianCalendar does) is
	 * ignored.
	 * <p>
	 * The recognised object types are defined in
	 * {@link org.joda.time.convert.ConverterManager ConverterManager} and
	 ...
	 */
	// @ requires year > 0;
	// @ requires monthOfYear > 0 && monthOfYear < 13;
	// @ requires dayOfMonth > 0 && dayOfMonth < 31;
	// @ requires hourOfDay >= 0 && hourOfDay <= 24;
	// @ requires minuteOfHour >= 0 && minuteOfHour <= 60;
	...
	/**
	 * Constructs an instance from datetime field values using the specified
	 * chronology.
	 * <p>
	 * If the chronology is null, <code>ISOChronology</code> in the default time
	 * zone is used.
	 ...
	 * @param hourOfDay
	 *            the hour of the day, valid values defined by the chronology
	 * @param minuteOfHour
	...
	// @ requires hourOfDay >= 0 && hourOfDay <= 24;
	// @ requires minuteOfHour >= 0 && minuteOfHour <= 60;
	// @ requires secondOfMinute >= 0 && secondOfMinute <= 60;
	 ...
	 *            the minute of the hour, valid values defined by the chronology
	 * @param secondOfMinute
	 *            the second of the minute, valid values defined by the
	 *            chronology
	 * @param chronology
	 *            the chronology, null means ISOChronology in default zone
	 * @since 2.0
	...
	// @ requires minuteOfHour >= 0 && minuteOfHour <= 60;
	// @ requires secondOfMinute >= 0 && secondOfMinute <= 60;
	// @ requires millisOfSecond >= 0 && millisOfSecond <= 1000;
	public DateTime(int year, int monthOfYear, int dayOfMonth, int hourOfDay, int minuteOfHour, int secondOfMinute,
			int millisOfSecond) {
	 ...
	 *            chronology
	 * @param millisOfSecond
	 *            the millisecond of the second, valid values defined by the
	 *            chronology
	 * @param chronology
	 *            the chronology, null means ISOChronology in default zone
	 */
	 ...
	 * <code>this</code>. Only the millis will change, the chronology and time
	 * zone are kept.
	 *
	 * @param newMillis
	 ...
	 * <p>
	 * The returned object will be either be a new instance or
	 * <code>this</code>. Only the chronology will change, the millis are kept.
	 *
	 * @param newChronology
	 ...
	 * <p>
	 * This method is useful for finding the local time in another timezone. For
	 * example, if this instant holds 12:30 in Europe/London, the result from
	 * this method with Europe/Paris would be 13:30.
	 * <p>
	 * The returned object will be a new instance of the same implementation
	 * type. This method changes the time zone, and does not change the
	 * millisecond instant, with the effect that the field values usually
	 * change. The returned object will be either be a new instance or
	 * <code>this</code>.
	 *
	 * @param newZone
	 *            the new time zone
	 ...
	 * the field values.
	 * <p>
	 * This method is useful for finding the millisecond time in another
	 * timezone. For example, if this instant holds 12:30 in Europe/London (ie.
	 * 12:30Z), the result from this method with Europe/Paris would be 12:30
	 * (ie. 11:30Z).
	 * <p>
	 * The returned object will be a new instance of the same implementation
	 * type. This method changes the time zone and the millisecond instant to
	 * keep the field values the same. The returned object will be either be a
	 * new instance or <code>this</code>.
	 *
	 * @param newZone
	 ...
	 * <p>
	 * This method only has any effect when the local time-line overlaps, such
	 * as at an autumn daylight savings cutover. In this scenario, there are two
	 * valid offsets for the local date-time. Calling this method will return a
	 ...
	 *
	 * @return a copy of this datetime with the earliest valid offset for the
	 *         local datetime
	 */
	 ...
	 * This instance is immutable and unaffected by this method call.
	 *
	 * @return a copy of this datetime with the latest valid offset for the
	 *         local datetime
	 */
	 ...
	 * time fields.
	 * <p>
	 * If the date is already the date passed in, then <code>this</code> is
	 * returned.
	 * <p>
	 ...
	 * </pre>
	 * <p>
	 * If the time is invalid on the new date due to the time-zone, the time
	 * will be adjusted.
	 * <p>
	 * This instance is immutable and unaffected by this method call.
	 *
	 ...
	 * @return a copy of this datetime with a different date
	 * @throws IllegalArgumentException
	 *             if the time-of-day is invalid for the date
	 ...
	 * date fields.
	 * <p>
	 * If the time is already the time passed in, then <code>this</code> is
	 * returned.
	 * <p>
	 * To set a single field use the properties, for example:
	 * 
	 ...
	 * </pre>
	 * <p>
	 * If the new time is invalid due to the time-zone, the time will be
	 * adjusted.
	 * <p>
	 * This instance is immutable and unaffected by this method call.
	 *
	...
	public DateTime withTime(int hourOfDay, int minuteOfHour, int secondOfMinute, int millisOfSecond) {
		Chronology chrono = getChronology();
		long localInstant = chrono.withUTC().getDateTimeMillis(getYear(), getMonthOfYear(), getDayOfMonth(), hourOfDay,
	 ...
	 * This instance is immutable and unaffected by this method call.
	 *
	 * @return a copy of this datetime with the time set to the start of the
	 *         day, not null
	 */
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a copy of this datetime with the partial set of fields replacing
	 * those from this instance.
	 * <p>
	 * For example, if the partial is a <code>TimeOfDay</code> then the time
	 * fields would be changed in the returned instance. If the partial is null,
	 * then <code>this</code> is returned.
	 *
	 * @param partial
	 *            the partial set of fields to apply to this datetime, null
	 *            ignored
	...
	/**
	 * Returns a copy of this datetime with the specified field set to a new
	 * value.
	 * <p>
	 * For example, if the field type is <code>hourOfDay</code> then the hour of
	 * day field would be changed in the returned instance. If the field type is
	 * null, then <code>this</code> is returned.
	 * <p>
	...
	/**
	 * Returns a copy of this datetime with the value of the specified field
	 * increased.
	 * <p>
	 * If the addition is zero or the field is null, then <code>this</code> is
	 * returned.
	 * <p>
	 * These three lines are equivalent:
	 * 
	 ...
	 * instances. Adding one field is best achieved using methods like
	 * {@link #withFieldAdded(DurationFieldType, int)} or
	 * {@link #plusYears(int)}.
	 * 
	 * @param period
	 ...
	 * Returns a copy of this datetime with the specified duration added.
	 * <p>
	 * If the amount is zero or null, then <code>this</code> is returned. This
	 ...
	 * <p>
	 * Thus, adding a period of one month and one day to 2007-03-31 will work as
	 * follows: First add one month and adjust, resulting in 2007-04-30 Then add
	 * one day and adjust, resulting in 2007-05-01.
	 * <p>
	 * This method is typically used to add complex period instances. Adding one
	 * field is best achieved using methods like {@link #plusYears(int)}.
	 * <p>
	 * If the amount is zero or null, then <code>this</code> is returned. This
	 * datetime instance is immutable and unaffected by this method call.
	 * 
	 ...
	 * <p>
	 * The calculation will do its best to only change the year field retaining
	 * the same month of year. However, in certain circumstances, it may be
	 * necessary to alter smaller fields. For example, 2008-02-29 plus one year
	 * cannot result in 2009-02-29, so the day of month is adjusted to
	 * 2009-02-28.
	 * <p>
	 ...
	 * <p>
	 * The calculation will do its best to only change the month field retaining
	 * the same day of month. However, in certain circumstances, it may be
	 * necessary to alter smaller fields. For example, 2007-03-31 plus one month
	 * cannot result in 2007-04-31, so the day of month is adjusted to
	 * 2007-04-30.
	 * <p>
	 * The following three lines are identical in effect:
	 * 
	 ...
	 * <p>
	 * The calculation will do its best to only change the day field retaining
	 ...
	 * <p>
	 * In spring an hour is typically removed. If adding one day results in the
	 * time being within the cutover then the time is adjusted to be within
	 * summer time. For example, if the cutover is from 01:59 to 03:00 and the
	 ...
	 * <p>
	 * The calculation will add a duration equivalent to the number of
	 * milliseconds.
	 * <p>
	 * The following three lines are identical in effect:
	 * 
	 ...
	 * work as follows: First subtract one month and adjust, resulting in
	 * 2007-04-30 Then subtract one day and adjust, resulting in 2007-04-29.
	 * Note that the day has been adjusted by two.
	 * <p>
	 * This method is typically used to subtract complex period instances.
	 * Subtracting one field is best achieved using methods like
	 * {@link #minusYears(int)}.
	 * <p>
	 * If the amount is zero or null, then <code>this</code> is returned. This
	 * datetime instance is immutable and unaffected by this method call.
	 ...
	 * The calculation will do its best to only change the year field retaining
	 * the same month of year. However, in certain circumstances, it may be
	 * necessary to alter smaller fields. For example, 2008-02-29 minus one year
	 * cannot result in 2007-02-29, so the day of month is adjusted to
	 * 2007-02-28.
	 * <p>
	 * The following three lines are identical in effect:
	 * 
	 ...
	 * The calculation will do its best to only change the month field retaining
	 * the same day of month. However, in certain circumstances, it may be
	 * necessary to alter smaller fields. For example, 2007-05-31 minus one
	 * month cannot result in 2007-04-31, so the day of month is adjusted to
	 * 2007-04-30.
	 * <p>
	 * The following three lines are identical in effect:
	 * 
	 ...
	 * <p>
	 * The calculation operates as if it were subtracting the equivalent in
	 * days.
	 * <p>
	 * The following three lines are identical in effect:
	 * 
	 ...
	 * daylight savings cutover, it may be necessary to alter the time fields.
	 * <p>
	 * In spring an hour is typically removed. If subtracting one day results in
	 * the time being within the cutover then the time is adjusted to be within
	 * summer time. For example, if the cutover is from 01:59 to 03:00 and the
	 * result of this method would have been 02:30, then the result will be
	 * adjusted to 03:30.
	 * <p>
	...
	// -----------------------------------------------------------------------
	/**
	 * Converts this object to a <code>DateMidnight</code> using the same millis
	 * and chronology.
	 * 
	...
	/**
	 * Converts this object to a <code>YearMonthDay</code> using the same millis
	 * and chronology.
	 * 
	...
	/**
	 * Converts this object to a <code>TimeOfDay</code> using the same millis
	 * and chronology.
	 * 
	...
	/**
	 * Converts this object to a <code>LocalDateTime</code> with the same
	 * datetime and chronology.
	 *
	...
	/**
	 * Converts this object to a <code>LocalDate</code> with the same date and
	 * chronology.
	 *
	...
	/**
	 * Converts this object to a <code>LocalTime</code> with the same time and
	 * chronology.
	 *
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of era changed.
	 *
	 * @param era
	 *            the era to set
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of century of era changed.
	 *
	 * @param centuryOfEra
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of year of era changed.
	 *
	 * @param yearOfEra
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of year of century changed.
	 *
	 * @param yearOfCentury
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of year changed.
	 *
	 * @param year
	 *            the year to set
	 * @return a copy of this object with the field set
	 ...
	 * <p>
	 * The weekyear is the year that matches with the weekOfWeekyear field. In
	 * the standard ISO8601 week algorithm, the first week of the year is that
	 * in which at least 4 days are in the year. As a result of this definition,
	 * day 1 of the first week may be in the previous year. The weekyear allows
	 * you to query the effective year for that day.
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of weekyear changed.
	 *
	 * @param weekyear
	 *            the weekyear to set
	 * @return a copy of this object with the field set
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of month of year changed.
	 *
	 * @param monthOfYear
	 ...
	 * {@link #withWeekyear(int)}. In the standard ISO8601 week algorithm, the
	 * first week of the year is that in which at least 4 days are in the year.
	 * As a result of this definition, day 1 of the first week may be in the
	 * previous year.
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of week of weekyear changed.
	 *
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of day of year changed.
	 *
	 * @param dayOfYear
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of day of month changed.
	 *
	 * @param dayOfMonth
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of day of week changed.
	 *
	 * @param dayOfWeek
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of hour of day changed.
	 *
	 * @param hour
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of minute of hour changed.
	 *
	 * @param minute
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of second of minute changed.
	 *
	 * @param second
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of millis of second changed.
	 *
	 * @param millis
	 ...
	 * <p>
	 * DateTime is immutable, so there are no set methods. Instead, this method
	 * returns a new instance with the value of millis of day changed.
	 *
	 * @param millis
	...
	/**
	 * Get the century of era property which provides access to advanced
	 * functionality.
	 * 
	...
	/**
	 * Get the year of century property which provides access to advanced
	 * functionality.
	 * 
	 * @return the year of era property
	 */
	...
	/**
	 * Get the year of era property which provides access to advanced
	 * functionality.
	 * 
	 * @return the year of era property
	 */
	...
	/**
	 * Get the year of a week based year property which provides access to
	...
	/**
	 * Get the month of year property which provides access to advanced
	 * functionality.
	 * 
	...
	/**
	 * Get the week of a week based year property which provides access to
	 * advanced functionality.
	 * 
	...
	/**
	 * Get the day of year property which provides access to advanced
	 * functionality.
	 * 
	...
	/**
	 * Get the day of month property which provides access to advanced
	 * functionality.
	 * 
	...
	/**
	 * Get the day of week property which provides access to advanced
	 * functionality.
	 * 
	...
	/**
	 * Get the minute of day property which provides access to advanced
	 * functionality.
	 * 
	...
	/**
	 * Get the second of day property which provides access to advanced
	 * functionality.
	 * 
	 ...
	 * <p>
	 * The simplest use of this class is as an alternative get method, here used
	 * to get the year '1972' (as an int) and the month 'December' (as a
	 * String).
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * <p>
	 * Methods are also provided that allow date modification. These return new
	 * instances of DateTime - they do not modify the original. The example
	 ...
	 * </pre>
	 * 
	 * Serious modification of dates (ie. more than just changing one or two
	 * fields) should use the {@link org.joda.time.MutableDateTime
	...
	public static final class Property extends AbstractReadableInstantFieldProperty {
		...
		/**
		 * Gets the milliseconds of the datetime that this property is linked
		 * to.
		 * 
		 ...
		 * This operation is faster than converting a DateTime to a
		 * MutableDateTime and back again when setting one field. When setting
		 * multiple fields, it is generally quicker to make the conversion to
		 * MutableDateTime.
		 * 
		 * @param value
		...
		/**
		 * Adds to this field, possibly wrapped, in a copy of this DateTime. A
		 * wrapped operation only changes this field. Thus 31st January
		 * addWrapField one day goes to the 1st January.
		 * <p>
		 * The DateTime attached to this property is unchanged by this call.
		 * This operation is faster than converting a DateTime to a
		 * MutableDateTime and back again when setting one field. When setting
		...
		// -----------------------------------------------------------------------
		/**
		 * Returns a new DateTime with this field set to the maximum value for
		 * this field.
		 * <p>
		 * This operation is useful for obtaining a DateTime on the last day of
		 * the month, as month lengths vary.
		 * 
		 * <pre>
		 * DateTime lastDayOfMonth = dt.dayOfMonth().withMaximumValue();
		 * </pre>
		 * <p>
		 * Where possible, the offset from UTC will be retained, thus
		 * applications may need to call
		 * {@link DateTime#withLaterOffsetAtOverlap()} on the result to force
		 * the later time during a DST overlap if desired.
		 * <p>
		 * From v2.2, this method handles a daylight savings time gap, setting
		 * the time to the last instant before the gap.
		 * <p>
		 * The DateTime attached to this property is unchanged by this call.
		 *
		...
		public DateTime withMaximumValue() {
			try {
				...
				if (IllegalInstantException.isIllegalInstant(ex)) {
					// adding MILLIS_PER_DAY is not perfect, but will work in
		...
		/**
		 * Returns a new DateTime with this field set to the minimum value for
		 * this field.
		 * <p>
		 * Where possible, the offset from UTC will be retained, thus
		 * applications may need to call
		 * {@link DateTime#withEarlierOffsetAtOverlap()} on the result to force
		 * the earlier time during a DST overlap if desired.
		 * <p>
		 * From v2.2, this method handles a daylight savings time gap, setting
		 * the time to the first instant after the gap.
		 * <p>
		 * The DateTime attached to this property is unchanged by this call.
		 *
		...
		public DateTime withMinimumValue() {
			try {
				...
				if (IllegalInstantException.isIllegalInstant(ex)) {
					// subtracting MILLIS_PER_DAY is not perfect, but will work
					// in almost all situations
					long afterGap = getChronology().getZone()
		 ...
		 * DateTime, favoring the floor if halfway.
		 *
		 * @return a copy of the DateTime with the field value changed
		 */
		...
		/**
		 * Rounds to the nearest whole unit of this field on a copy of this
		 * DateTime, favoring the ceiling if halfway.
		 *
		 * @return a copy of the DateTime with the field value changed
		 */
